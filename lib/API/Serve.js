...
var options = {
  ...
  basic_auth:
    process.env.PM2_SERVE_BASIC_AUTH === "true"
      ? {
...
if (typeof options.monitor === "string" && options.monitor !== "") {
  try {
    let fileContent = fs
      .readFileSync(path.join(process.env.PM2_HOME, "agent.json5"))
      .toString();
...
http
  .createServer(function (request, response) {
    if (options.basic_auth) {
      ...
      if (
        user.username !== options.basic_auth.username ||
        user.password !== options.basic_auth.password
      ) {
        return sendBasicAuthResponse(response);
      }
    }
...
function serveFile(uri, request, response) {
  ...
  var contentType =
    contentTypes[filePath.split(".").pop().toLowerCase()] || "text/plain";

  fs.readFile(filePath, function (error, content) {
    if (error) {
      if (!options.spa || file === options.homepage) {
        console.error(
          "[%s] Error while serving %s with content-type %s : %s",
          new Date(),
          filePath,
          contentType,
          error.message || error
...
`
    ...
    debug(
      "[%s] Serving %s with content-type %s",
      Date.now(),
      filePath,
      contentType
    );
