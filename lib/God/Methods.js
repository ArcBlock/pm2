  ...
  God.processIsDead = function(pid, pm2_env, cb, sigkill) {
    ...
    var timer = setInterval(function() {
      if (God.checkProcess(pid) === false) {
        console.log('pid=%d name=%s msg=process killed', pid, pm2_env.name);
        clearTimeout(timeout);
        clearInterval(timer);
        return cb(null, true);
      }
      console.log('pid=%d name=%s msg=failed to kill - retrying in %dms', pid, pm2_env.name, pm2_env.kill_retry_time);
      return false;
